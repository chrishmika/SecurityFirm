...
const NotificationCard = ({ notification }) => {
  ...
  return (
    <div className="flex justify-center items-center  ">
      <div className={`flex border-b-2 pb-3 justify-between sm:w-4xl w-screen m-1 px-5 cursor-pointer ${!isRead ? "border-l-6  border-l-lime-200" : "border-l-6"}`} key={_id}>
        ...
        <div className="flex md:gap-10 items-center flex-row gap-3">
          {favourites ? <FaStar className=" m-1.5 text-2xl" onClick={favBtnHandler} /> : <FaRegStar className=" m-1.5 text-2xl" onClick={favBtnHandler} />}
          <div className="flex md:flex-row flex-col gap-2 ">
            <button onClick={viewNotificationhandler} className="bg-green-500 rounded-2xl px-4 py-2 text-white cursor-pointer">
              Read
            </button>
            <button onClick={deleteNotificationHandler} className="bg-red-500 rounded-2xl px-4 py-2 text-white cursor-pointer">
              Delete
            </button>
          </div>
        </div>
          ...
          <div className=" flex absolute md:inset-20 sm:inset-y-50 inset-y-20 inset-x-10 inset bg-red-400 ml-15 sm:mx-50">
            ...
            <div>
              <div className="flex flex-col justify-center m-5 w-md relative">
                ...
                <span className="text-gray-900 text-sm">Created At : {createdAt?.split("T")[0]}</span>
                ...
                <div className="flex md:flex-row flex-col gap-2 ">
                  {favourites ? <FaStar className="cursor-pointer m-1.5 text-2xl" onClick={favBtnHandler} /> : <FaRegStar className=" m-1.5 text-2xl cursor-pointer" onClick={favBtnHandler} />}
