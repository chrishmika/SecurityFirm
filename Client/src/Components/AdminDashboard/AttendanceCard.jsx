...
//need to cahne values when it taken from database
        ...
        <div className={`p-8  flex flex-col fixed inset-0 bg-black bg-opacity-50 items-center justify-center rounded-2xl text-white w-fit h-fit mx-auto my-auto ${editInterface ? "flex" : "hidden"}`}>
          ...
          <div className="flex gap-8 flex-col md:flex-row">
            <div className="flex flex-col gap-5 mt-5">
              <span>Check in -</span>
              <span>
                Date: <input type="date" className={inputFieldStyle} value={startDate} onChange={(e) => setStartDate(e.target.value)} />
              </span>
              <span>
                Time: <input type="time" className={inputFieldStyle} value={startTime} onChange={(e) => setStartTime(e.target.value)} />
              </span>
            </div>
            <div className="flex flex-col  gap-5 mt-5">
              <span>Check Out -</span>
              <span>
                Date: <input type="date" className={inputFieldStyle} value={endDate} onChange={(e) => setEndDate(e.target.value)} />
              </span>
              <span>
                Time: <input type="time" className={inputFieldStyle} value={endTime} onChange={(e) => setEndTime(e.target.value)} />
              </span>
            </div>
          </div>
